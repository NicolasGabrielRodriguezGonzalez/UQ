...
public class App {
    ...
    public static void main(String[] args) {
        ...
        Administrador administradorInicial = new Administrador(
                "Carlos Pérez", "A123", "3001234567", "carlos@admin.com", 
                "A123", "admin", "admin", // ID, usuario y contraseña
                true, LocalDate.of(2020, 1, 1), 5000000, 
                "¿Cuál es tu mascota?", "pacho"
             );
        administradores.add(administradorInicial);
    ...
    //Excepciones
    private static void crearCuentaAdministrador() {
        // Solicitar datos para crear la cuenta del administrador
        String idAdministrador = JOptionPane.showInputDialog("Ingrese un ID único para el Administrador:");
        ...
        if (respuestaSeguridad == null || respuestaSeguridad.isEmpty()) {
            JOptionPane.showMessageDialog(null, "La respuesta de seguridad es obligatoria.");
            return;
        }
    
        // Crear el nuevo administrador y agregarlo a la lista
        Administrador nuevoAdministrador = new Administrador(
                nombre, idAdministrador, "3001234567", "admin@domain.com",
                idAdministrador, usuario, contrasena,
                true, LocalDate.now(), 5000000,
    ...
    private static void iniciarSesionAdministrador() {
        ...
        String idAdministrador = JOptionPane.showInputDialog("Ingrese el ID del Administrador:");
        ...
        if (administrador != null) {
            ...
            JOptionPane.showMessageDialog(null, "ID de Administrador no encontrado.");
        }
    }

    private static Administrador buscarAdministradorPorId(String id) {
...
private static void mostrarMenuAdministrador() {
    ...
    while (true) {
        ...
        if (accion == null || accion.equals("3")) {
            ...
            // Permitir gestionar empleados sin importar si la lista está vacía
            gestionarEmpleados();
        } else if (accion.equals("2")) {
            // Solo mostrar reportes si hay empleados, si no, mostrar un mensaje
            if (!listaEmpleados.isEmpty()) {
                verReportes();
            } else {
                JOptionPane.showMessageDialog(null, "No hay empleados registrados para generar reportes.");
            }
        }
    }
}
    ...
    private static void gestionarEmpleados() {
        // Permitir acceso a la gestión de empleados incluso si la lista está vacía
        ...
        switch (opcion) {
            ...
            case 1: // Modificar Empleado
                // Solo permitir modificar si hay empleados registrados
                if (!listaEmpleados.isEmpty()) {
                    modificarEmpleado();
                } else {
            ...
            case 2: // Eliminar Empleado
                // Solo permitir eliminar si hay empleados registrados
                if (!listaEmpleados.isEmpty()) {
                    eliminarEmpleado();
                } else {
            ...
            case 3: // Ver Detalles de Empleado
                // Solo permitir ver detalles si hay empleados registrados
                if (!listaEmpleados.isEmpty()) {
                    verDetallesEmpleado();
                } else {
                    JOptionPane.showMessageDialog(null, "No hay empleados registrados para mostrar.");
                }
                break;
            default:
